...
La liste de vecteurs nommés que retournent les deux fonctions peuvent être
utilisés pour convertir des variables caractères d'un data.frame.
...
les lignes qui créent des variables à partir de formats (instructions de la
forme `"NEW = put(OLD, $var.);"`) pour générer le code R corrspondant :
...
- signaler des bugs ou des cas où le package ne fonctionne pas correctement
  (de préference [ici](https://git.stable.innovation.insee.eu/xkfzv9/fmtsas/issues))
